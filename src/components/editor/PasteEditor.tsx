...
export function PasteEditor({ config }: { config: PublicConfig }) {
  ...
  const overLimit =
    config.maxPlaintextBytes > 0 && byteLength > config.maxPlaintextBytes;
  ...
  async function handleFormat() {
    ...
    try {
      ...
      setNotice(
        cause instanceof Error ? cause.message : "Could not format this text.",
      );
    } finally {
      setFormatting(false);
    }
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      ...
      const encrypted = await encryptPaste(
        { content, language },
        password || undefined,
      );
  ...
  return (
    // The footer sits outside <main> on purpose: a <footer> nested inside main, article, or section
    // is not exposed as a `contentinfo` landmark, so screen-reader users would lose the page-footer
    // affordance entirely.
        ...
        <header className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Encrypted in your browser. The server stores ciphertext it cannot
              read.
            </p>
          </div>
          <ThemeToggle />
        ...
        <form onSubmit={handleSubmit} className="mt-6">
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-[1fr_auto_1fr_1fr]">
            ...
            <div className="grid gap-1.5">
              ...
              <Select
                value={expiryId}
                onValueChange={(value) => setExpiryId(value ?? expiryId)}
              >
          ...
          <Button
            type="submit"
            disabled={busy || content === "" || overLimit}
            className="mt-6"
          >
  ...
  return (
    <main className="mx-auto max-w-2xl px-6 py-20">
      ...
      <p className="mt-4 text-muted-foreground">
        ZeroPaste encrypts your text in the browser using the Web Crypto API,
        which browsers only expose over HTTPS or on localhost. This page was
        loaded over plain HTTP, so encryption is unavailable.
      </p>
      <p className="mt-4 text-muted-foreground">
        If you run this server, put it behind HTTPS. See{" "}
        <code>docs/AGENT-DEPLOY.md</code> §5.
      </p>
    </main>
  );
}
